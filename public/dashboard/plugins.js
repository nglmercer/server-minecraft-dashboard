import { awaitPlugins } from "../API/fetch.js";
const UI_SELECTORS = {
  PLUGINS: '#plugins-ui',
  MODS: '#mods-ui'
};
...
class awaitPluginsUI {
  ...
  static async downloadAndUploadFromURL(itemType, fileUrl) {
    ...
    const servername = awaitPluginsUI.getSelectedServer();
  ...
  static uploadItem(itemType) {
    ...
    const uploadURL = `/api/filemanager/upload?server=${getSelectedServer()}&path=${itemType}s`;
...
document.addEventListener('download-request', (e) => {
  ...
  if (details && details.files) {
    ...
    if (Array.isArray(details.files)) {
      ...
      awaitPluginsUI.downloadAndUploadFromURL(ismodorplugin, details.files[0]);
    } else {
      console.log("descargar array", details.files[0].url);
      awaitPluginsUI.downloadAndUploadFromURL(ismodorplugin, details.files[0].url);
    }

  }
});
//awaitPluginsUI.downloadAndUploadFromURL("plugin","https://github.com/minekube/connect-java/releases/download/latest/connect-spigot.jar");
...
class PluginsAndModsManager {
  ...
  async fetchPlugins() {
    return new Promise((resolve) => {
      awaitPlugins.getPluginsList((data) => {
...
const uploadItem_plugin = document.getElementById('uploadItem_plugin');
const uploadItem_mod = document.getElementById('uploadItem_mod');
// onclick="awaitPluginsUI.uploadItem('plugin')"
uploadItem_plugin.addEventListener('click', () => {
  console.log("uploadItem_plugin");
  awaitPluginsUI.uploadItem('plugin');
});
// onclick="awaitPluginsUI.uploadItem('mod')" 
uploadItem_mod.addEventListener('click', () => {
